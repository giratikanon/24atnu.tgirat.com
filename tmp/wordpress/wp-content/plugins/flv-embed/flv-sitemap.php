...
function flv_sitemap_options_panel()
{
	...
	if (!empty($options['last_built'])) {
		...
		if ($built)
			...
			$status = "Your <a href=\"$sitemap\" title=\"Video Sitemap\">video sitemap</a> was not properly built and it was last modified $last_built ago.";
	} else {
...
<div class="wrap">
		...
		<fieldset class="options">
			<legend>Video Sitemap Settings</legend>
				<p>If you see this page, it means sitemap feature is enabled for your <a href="./options-general.php?page=flv-embed.php">FLV Embed</a> plugin. For duration detection to work properly, FLV files must be hosted on the same server as your blog. Site relative path to your FLV file will be appended to the server path below when trying to fetch the FLV for duration detection.</p>
		...
		<fieldset class="options">
			<legend>Build Video Sitemap</legend>
				<p>Clicking the following button will generate a new video sitemap. {$status}</p>
		</fieldset>
		
		<p class="submit">
		<input type="submit" name="force_ping" value="Force Ping Now &raquo;" />
		<input type="submit" name="sitemap_build" value="Build Video Sitemap Now &raquo;" />
		</p>
		
		<fieldset class="options">
			<legend>Update Custom Fields</legend>
		...
		<p class="submit">
		<input type="submit" name="update_custom_fields" value="Update Custom Fields Now &raquo;" />
		</p>
...
<url>
	...
	<video:video>
		...
		<video:description>{$excerpt}</video:description>{$family_friendly}{$duration}
	</video:video>
</url>
